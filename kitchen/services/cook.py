...
def _prepare_cook(recipe, portions):
    ...
    return recipe, portions, locked, preview


def _consume_preview_into_batch(*, batch, preview, locked, user, recipe, portions):
...
def _restore_batch_stock(batch, user):
...
def cancel_cook_batch(*, batch, user=None):
    ...
    if batch.items.exists():
        _restore_batch_stock(batch, user)
...
def start_queued_cook(*, batch, user=None):
    """Navbatdagi (allaqachon rezervlangan) batchni DONE qiladi."""
    ...
    if not batch.items.exists():
        recipe, portions, locked, preview = _prepare_cook(batch.recipe, batch.portions)
    ...
    batch.cooked_at = timezone.now()
    batch.save(update_fields=['status', 'cooked_at'])
    log_action(user, 'pishirish', 'cook_batch', batch.pk, f'{batch.recipe.name} × {batch.portions}')
    return batch
